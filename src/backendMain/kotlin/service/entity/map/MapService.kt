...
class MapService<K : Any, V : Any>(
  private val targetDescriptor: TargetDescriptor,
  private val mapDescriptor: MapDescriptor<K, V>
) {
  init {
  ...
  companion object {
    fun instanceTag(mapDescriptor: MapDescriptor<*, *>) =
      "${mapDescriptor.environment}-${mapDescriptor.target}-${mapDescriptor.name}"
  }
}