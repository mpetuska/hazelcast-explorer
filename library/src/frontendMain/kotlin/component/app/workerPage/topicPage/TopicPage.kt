        ...
        synchronousButton {
          attrs {
            ...
            onClick = {
              if (props.subscribed) {
                store.dispatch(HzeAction.SetTopicSubscription(null))
                NotificationService.error("Unsubscribed to Topic [${props.selectedTopic?.displayName}]")
              } else {
                val subscription = TopicService.subscribe(props.selectedTopic) {
                  store.dispatch(HzeAction.AddTopicMessage(it))
                  NotificationService.info("[${Date(Date.now()).let { d -> "${d.getHours()}:${d.getMinutes()}:${d.getSeconds()}" }}] Message received for Topic [${props.selectedTopic?.displayName}]")
                }
                store.dispatch(HzeAction.SetTopicSubscription(subscription))
                NotificationService.success("Subscribed to Topic [${props.selectedTopic?.displayName}]")
              }
              it()
            }
          }
